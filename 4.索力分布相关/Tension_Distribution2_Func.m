function [t,bool] = Tension_Distribution2_Func(p,theta,we)
% 求解索力分布的函数
% 输入：p 动平台位置 3*1
%      theta 动平台坐标系XYZ欧拉角 3*1
%      we 动平台受到的wrench 6*1(包含本身质量)
% 输出：t 绳索的张力的大小 8*1
...
if count==4
    ...
    vc2=(langda_final_vce(2,1)+langda_final_vce(2,2)+langda_final_vce(2,3)+langda_final_vce(2,4))/3;
end
